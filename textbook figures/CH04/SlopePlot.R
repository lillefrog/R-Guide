...
myData <- data.frame(
  Name = c("Combustible fuels", "Hydro", "Wind", "Solar", "Nuclear fuels", "other"),   # Name of each data point
...
png(file = "CH4_slopePlot.png", units="in", width=11, height=7, res=300)

# Set up the plot window with extra x-margin and no x-axis
par(mar=c(4, 5, 4, 2)) # Bottom, Left, Top, Right
...
plot(c(1, 2),                                # set some default x values to start with
     ...
     bty="n", cex.lab=1.5, cex.axis=1.2              # Draw box around plot (n=none, )
     )
...
for(i in 1:nrow(myData)) {
  ...
  text(1, myData$year2012[i], labels=myData$Name[i], pos=2, cex=1.2)           # label the points on the left side
  text(2, myData$year2022[i], labels=myData$Name[i], pos=4, cex=1.2)           # label the points on the right side
}

# Add custom labels instead of axis
text(x=1, y=par("usr")[3] - 100, labels="2012", cex=1.5, xpd=NA) # Draw the text under the figure
text(x=2, y=par("usr")[3] - 100, labels="2022", cex=1.5, xpd=NA)  # xpd=NA allows drawing the text outside the plot region, but still inside the figure region
