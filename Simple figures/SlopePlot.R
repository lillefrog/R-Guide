...
# Sample data set insert your ovw data here
myData <- data.frame(
  Name = c("A", "B", "C", "D"),
...
plot(c(1, 2), 
     range(c(myData$Value1, myData$Value2)), # set the y-axis to go from the lowest to the higest value in our dataset
     type="n",            # type of points to draw (n=none)
     xaxt="n",            # type of x-axis to draw (n=none)
     xlab="",             # Lable on the x axis
     ylab="Value",        # Lable on the y axis 
...
for(i in 1:nrow(myData)) {
  ...
  # You can remove the next two lines if you don't want to lable points
  text(1, myData$Value1[i], labels=myData$Name[i], pos=2)           # Lable the points on the left side
  text(2, myData$Value2[i], labels=myData$Name[i], pos=4)           # Lable the points on the right side
}
